...
def main(flags, lidar_file_path):
    ...
    img_name = lidar_file_path.split('/')[-1][:-4] +  'x_' + str(flags.rx) + '_y_' + str(flags.ry) + '_z_' + str(flags.rz) + '_scale_' + str(flags.scale) + '.png'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rx", type=int, default=65, help='',)
    parser.add_argument("--ry", type=int, default=0 , help='',)
    parser.add_argument("--rz", type=int, default=354, help='', )#lidarxyzintensityright000150.bin
    parser.add_argument("--scale", type=float, default=1., help="")


    flags = parser.parse_args()
    lidar_files = glob.glob(flags.lidarfile + '/*')
    lidar_files.sort()
    for l_f in lidar_files:
        main(flags, l_f)
