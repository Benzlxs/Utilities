...
def mesh_line_intersection(inputs):
    mesh_vertices, mesh_triangles, line_origin, line_direction  = inputs
    intersections = []
    ...
    for triangle in mesh_triangles:
        ...
        intersection_point = ray_triangle_intersection(line_origin, line_direction, v0, v1, v2)
...
def ray_triangle_intersection_batch(inputs):
    vertices, triangles, ray_origin, ray_direction = inputs
    epsilon = 1e-6
    ...
    u = np.einsum('ij,ij->i', s, h[mask]) * f

    mask = np.logical_and(u >= 0.0, u <= 1.0)
    valid_triangles = valid_triangles[mask]

    q = np.cross(s[mask], edge1[mask])
    v = np.einsum('ij,ij->i', ray_direction[:,None], q) * f

    mask = np.logical_and(v >= 0.0, u + v <= 1.0)
    valid_triangles = valid_triangles[mask]

    t = np.einsum('ij,ij->i', edge2[valid_triangles - 1], q[mask])

    mask = t > epsilon
    valid_triangles = valid_triangles[mask]
    t = t[mask]

    intersection_points = ray_origin + t[:, np.newaxis] * ray_direction
...
def main(img_idx, args):
    ...
    # mesh reading
    # mesh = o3d.io.read_triangle_mesh('bpa_mesh.ply')
    # vertices = np.asarray(mesh.vertices)
    # triangles = np.asarray(mesh.triangles)
    mesh = trimesh.load_mesh('bpa_mesh.ply')
    # vertices = np.asarray(mesh.vertices)
    # pc_scale_mat = scale_mats_np[0]
    # radius = pc_scale_mat[0,0]
    # pc_translation = pc_scale_mat[:3,3]
    # vertices -= pc_translation
    # vertices /= radius
    ...
    for h_start in range(0, H_img, h_gap):
        ...
        # intersections = mesh_line_intersection((vertices, triangles, line_point, rays_v[1,:]))
        # intersections = ray_triangle_intersection_batch((vertices, triangles, line_point, rays_v[1,:]))
        ray_origins = np.repeat(line_point[None,:], rays_v.shape[0], axis=0)
        locations, index_ray, index_tri = mesh.ray.intersects_location(ray_origins=rays_o, ray_directions=rays_v)
        print("Effecitve points:", locations.shape)
        all_z_dist = np.linalg.norm(locations - line_point[None,:], ord=2,axis=-1, keepdims=True)
        for idx_ray in range(index_ray.shape[0]):
            p_idx = index_ray[idx_ray]
            p_z_val = all_z_dist[idx_ray]
            p_z_val_cur = z_val_array[all_pix_y[p_idx] - h_start, all_pix_x[p_idx]]
            z_val_array[all_pix_y[p_idx] - h_start, all_pix_x[p_idx]] = min(p_z_val, p_z_val_cur)
